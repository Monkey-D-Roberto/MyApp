...
import backgrounds from '../../assets/login-background.jpg';
import logo from '../../assets/Roberto.png';
...
const handleLogin = () => {
  ...
  return (
    <div className='flex md:flex-row flex-col h-screen justify-center items-center'>
      ...
      {!isAuthenticated && (
        <>
        <p className='italic text-sm text-stone-500 text-center font-bold'>** Inicia tu sesión **</p>
        ...
        {error && <p className='text-red-500 mt-4 text-sm flex flex-col'>Resp: {error} <a className='text-emerald-800'> * Por favor verifique su nombre de usuario y contraseña.</a></p>}
        </>
      )}
      {isAuthenticated && (
        <>
        <div>
          ...
          />
          <LabelMessage text='¡Inicio de sesión correcto! Se le dirige a la aplicación.'/>
      </div>
        </>
      )}
